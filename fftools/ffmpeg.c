                          ...
                          OutputStream *ost, int eof)
{
    ...
    if (ost->nb_bitstream_filters) {
        ...
        ret = av_bsf_send_packet(ost->bsf_ctx[0], eof ? NULL : pkt);   //提交数据包进行过滤
        if (ret < 0)
            goto finish;
                        ...
                        int *decode_failed)
{
    ...
    if (ist->dts != AV_NOPTS_VALUE)
        dts = av_rescale_q(ist->dts, AV_TIME_BASE_Q, ist->st->time_base);  //解码时间戳转换
    if (pkt) {
        avpkt = *pkt;
    ...
    update_benchmark(NULL);
    ret = decode(ist->dec_ctx, decoded_frame, got_output, pkt ? &avpkt : NULL);    //解码函数
    ...
    *duration_pts = decoded_frame->pkt_duration;
    ...
    err = send_frame_to_filters(ist, decoded_frame);   //将解码后的帧送到滤镜中
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
    ...
    */
    ...
    while (ist->decoding_needed) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ret = decode_video    (ist, repeating ? NULL : &avpkt, &got_output, &duration_pts, !pkt,
                                   &decode_failed);
            if (!repeating || !pkt || got_output) {  //计算视频的总时长
                if (pkt && pkt->duration) {
                    duration_dts = av_rescale_q(pkt->duration, ist->st->time_base, AV_TIME_BASE_Q);   //时间基准的转换
    ...
    /* handle stream copy */ //不需要重新编解码，只需要进行时间基的转换
...
static int process_input(int file_index)
{
    ...
    is  = ifile->ctx;
    ret = get_input_packet(ifile, &pkt);  //获取包，可能是一帧视频也可能是几帧音频，里面调用了av_read_frame函数读取包avpkt
   ...
   /////////////////////////////////////////////////////////////////////////////////////////////////// 无用
    ...
    ist = input_streams[ifile->ist_index + pkt.stream_index];   //InputStream结构体填充

    ist->data_size += pkt.size;    //统计所有packet的大小
    ist->nb_packets++;             //统计packet的数量
...
static int transcode_step(void)
{
   ...
   //判断是否配置了滤镜
    ...
    ret = process_input(ist->file_index);  //完成解码操作
    if (ret == AVERROR(EAGAIN)) {
        if (input_files[ist->file_index]->eagain)
            ost->unavailable = 1;
        return 0;
...
#if HAVE_THREADS
    if ((ret = init_input_threads()) < 0)   //根据输入文件的数量产生新的进程来处理这些输入
        goto fail;
#endif
   //循环，直到收到系统信号才退出
    while (!received_sigterm) {
        ...
        /* if 'q' pressed, exits */
        if (stdin_interaction)
            if (check_keyboard_interaction(cur_time) < 0)  //检测键盘操作，如果在转码过程中有Q，则退出
                break;
    ...
    for (i = 0; i < nb_input_streams; i++) {
        ...
        if (!input_files[ist->file_index]->eof_reached) {
            process_input_packet(ist, NULL, 0);//该函数对帧数据进行解码并通过所有适用的过滤器进行处理。
                                               //时间戳校准和字幕处理的工作也在这个函数中进行。
        }
    }
    flush_encoders();
