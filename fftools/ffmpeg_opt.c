...
static int open_input_file(OptionsContext *o, const char *filename)  //输入参数o里面存放着文件的对应参数，filename表示文件名
{
    ...
    /* apply forced codec ids 找到解码器，如果命令行里有指定强制解码类型就用之，否则根据流中的st->codec->codec_id找*/
    for (i = 0; i < ic->nb_streams; i++)
        choose_decoder(o, ic, ic->streams[i]);

    if (find_stream_info) {
		/*返回的opts为一个链表，它的成员个数和本输入文件的流个数相同，opts[0]表示stream[0]对应的参数，来源是o->g->codec_opts
    也就是将一个文件中的视频流参数、音频流参数、字幕流参数和数据流参数分开，之后提供给avformat_find_stream_info()函数用*/
    ...
    /* if seeking requested, we execute it */ //如果开始时间指定的话，需要做seek
	...
	/*将本文件的各个流放入input_streams链表中，并对解码器进行设置
     * InputStream中的项在此函数中填值 */
    add_input_streams(o, ic);

    /* dump the file content */
    av_dump_format(ic, nb_input_files, filename, 0);
    /*将这个文件放到input_files链表中，并对其中的变量做设置*/
...
static int open_output_file(OptionsContext *o, const char *filename)
{
    ...
    /* create streams for all unlabeled output pads */  //主要涉及到滤镜的处理
    for (i = 0; i < nb_filtergraphs; i++) {
    ...
    if (!o->nb_stream_maps) {
        ...
        if (!o->video_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_VIDEO) != AV_CODEC_ID_NONE) {
            int area = 0, idx = -1;
			//检查是否此容器能存储这个编码的流，返回1为能，0为不能，负数为信息不可用
            ...
            if (idx >= 0)
                new_video_stream(o, oc, idx);   //为输出文件添加一路视频流
        }

        /* audio: most channels */  //处理音频
        ...
        /* Data only if codec id match */   //处理数据流
        if (!o->data_disable ) {
                      ...
                      int (*open_file)(OptionsContext*, const char*))
{
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        ret = open_file(&o, g->arg);   //g->arg中存放的是输入文件路径
        uninit_options(&o);
        if (ret < 0) {
