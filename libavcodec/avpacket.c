...
/*从这个函数也可以看到，如果某个packet有多个ref，那么这个packet的数据其实有两份存储位置，
一份就是这个packet自己data指针所指向，另一份是这个packet的其他所有ref
...
int av_packet_ref(AVPacket *dst, const AVPacket *src)
{
    ...
    if (!src->buf) {    //如果源packet（即src）本身不是一个ref，则这里相当于要生成它的第一个ref，
                        //AVPacket里面的buf成员是否为null，表明的就是这个packet是否是一个ref
