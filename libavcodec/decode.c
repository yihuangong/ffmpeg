...
int ff_decode_get_packet(AVCodecContext *avctx, AVPacket *pkt)
{
    ...
    ret = bsfs_poll(avctx, pkt);  //将avctx中的pkt拷贝到AVPacket中
    if (ret == AVERROR_EOF)
        avci->draining = 1;
    if (ret < 0)
        return ret;
...
static int decode_simple_internal(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (!pkt->data && !avci->draining) {   //只有解码结束时才不会进入其中
        av_packet_unref(pkt);
        ret = ff_decode_get_packet(avctx, pkt);  //将avpkt数据从avctx->internal->filter.bsf[0]->internal->buff_pkt中提取出到pkt中,其中包含流过滤器的操作
    ...
    if (HAVE_THREADS && avctx->active_thread_type & FF_THREAD_FRAME) {   //经过测试，不运行此处
        ...
        if (avctx->codec->type == AVMEDIA_TYPE_VIDEO) {
            if(!avctx->has_b_frames)
                frame->pkt_pos = pkt->pos;    //没有B帧，流中的位置也就是frame中的位置
            //FIXME these should be under if(!avctx->has_b_frames)
            /* get_buffer is supposed to set frame parameters */
            if (!(avctx->codec->capabilities & AV_CODEC_CAP_DR1)) {   //没有运行此处
...
static int decode_receive_frame_internal(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (avctx->codec->receive_frame)   //codec结构体中没有receive_frame这个指针
...
int attribute_align_arg avcodec_send_packet(AVCodecContext *avctx, const AVPacket *avpkt)
{
    ...
    //提交数据包进行过滤，把AVPacket的数据传给 avci->filter.bsfs[0]，会将AVPacket的数据清掉
