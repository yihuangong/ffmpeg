...
int ff_h264_queue_decode_slice(H264Context *h, const H2645NAL *nal)
{
    ...
    ret = h264_slice_header_parse(h, sl, nal);    //解析slice头部信息
    if (ret < 0)
        return ret;
    ...
    if (sl->first_mb_addr == 0) { // FIXME better field boundary detection
        if (h->current_slice) {   //判断当前slice的数量
            // this slice starts a new field
            // first decode any pending queued slices
            if (h->nb_slice_ctx_queued) {  //有slice正在排队等待解码
...
static void loop_filter(const H264Context *h, H264SliceContext *sl, int start_x, int end_x)
{
    ...
    if (sl->deblocking_filter) { 
        for (mb_x = start_x; mb_x < end_x; mb_x++)   //遍历每一个宏块进行环路滤波
            for (mb_y = end_mb_y - FRAME_MBAFF(h); mb_y <= end_mb_y; mb_y++) {
                ...
                if (FRAME_MBAFF(h)) {
                    ...
                    ff_h264_filter_mb_fast(h, sl, mb_x, mb_y, dest_y, dest_cb,
                                           dest_cr, linesize, uvlinesize);  //进行环路滤波
                }
            }
    }
...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    if (h->ps.pps->cabac) {
        ...
        for (;;) {
            ...
            // FIXME optimal? or let mb_decode decode 16x32 ? 宏块级帧场自适应，没有进入其中
            ...
            eos = get_cabac_terminate(&sl->cabac);   //终止符算术解码
            //该if为假
            ...
            if (++sl->mb_x >= h->mb_width) {       //解码完一行宏块后，进行环路滤波操作
                loop_filter(h, sl, lf_x_start, sl->mb_x);
                sl->mb_x = lf_x_start = 0;
                decode_finish_row(h, sl);       //结束一行的解码
                ++sl->mb_y;
                if (FIELD_OR_MBAFF_PICTURE(h)) {  //宏块级自适应，不进入
                    ++sl->mb_y;
            ...
            if (eos || sl->mb_y >= h->mb_height) {   //结束完成slice解码
                ff_tlog(h->avctx, "slice end %d %d\n",
                        get_bits_count(&sl->gb), sl->gb.size_in_bits);
                er_add_slice(sl, sl->resync_mb_x, sl->resync_mb_y, sl->mb_x - 1,
                             sl->mb_y, ER_MB_END);    //进行错误隐藏处理
                if (sl->mb_x > lf_x_start)
                    loop_filter(h, sl, lf_x_start, sl->mb_x);
                goto finish;
            }
