...
static av_cold int h264_decode_init(AVCodecContext *avctx)
{
    ...
    ret = h264_init_context(avctx, h); //初始化H264Context结构体
    if (ret < 0)
        return ret;
    ...
    if (avctx->ticks_per_frame == 1) {   //对于有些场编码，需要更改时间基
...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size)   //buf指向待解码数据
{
    ...
    if (h->nal_length_size == 4) {
        if (buf_size > 8 && AV_RB32(buf) == 1 && AV_RB32(buf+5) > (unsigned)buf_size) {
            ...
            h->is_avc = 1;         //该变量确定是否为avcc码流格式
    }
    
    ret = ff_h2645_packet_split(&h->pkt, buf, buf_size, avctx, h->is_avc,
                                h->nal_length_size, avctx->codec_id, avctx->flags2 & AV_CODEC_FLAG2_FAST);  //解析码流，得到NALU数据
    if (ret < 0) {
    ...
    for (i = 0; i < h->pkt.nb_nals; i++) {   //循环一个packet中的nal单元数量，一般是一个
        H2645NAL *nal = &h->pkt.nals[i];
        ...
        err = 0;
        switch (nal->type) {
        case H264_NAL_IDR_SLICE:    //是IDR SLICE同样会执行下面的SLICE解码操作
        ...
        case H264_NAL_SLICE:
            ...
            if (h->current_slice == 1) {
                if (avctx->active_thread_type & FF_THREAD_FRAME &&
                    i >= nals_needed && !h->setup_finished && h->cur_pic_ptr) {   //不执行
            ...
            if (h->nb_slice_ctx_queued == max_slice_ctx) {   //都是1
                if (h->avctx->hwaccel) {  //是否是硬件加速，否
...
static int h264_decode_frame(AVCodecContext *avctx, void *data,
                             int *got_frame, AVPacket *avpkt)  //avpkt是数据包，data是用来存储解码后的数据
{
    ...
    //decode_extradata在decode_init时就已经被解码，故在此处不运行，在此处主要是为防止新的extradata进来
    ...
    buf_index = decode_nal_units(h, buf, buf_size);  //buf指向待解码的数据，buf_size指的是avpacket的大小，一个avpacket是一帧视频或几帧音频
    if (buf_index < 0)
    ...
    if (!(avctx->flags2 & AV_CODEC_FLAG2_CHUNKS) ||
        ...
        if (h->next_output_pic) {
            ret = finalize_frame(h, pict, h->next_output_pic, got_frame);   //结束一帧的解码，等待第二帧，将解码后的数据放入pict中
            if (ret < 0)
                return ret;
        }
    }
    ...
    return get_consumed_bytes(buf_index, buf_size);  //获取解码该帧所花费的比特数
}
