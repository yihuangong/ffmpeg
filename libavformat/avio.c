...
static const struct URLProtocol *url_find_protocol(const char *filename)
{
    ...
    size_t proto_len = strspn(filename, URL_SCHEME_CHARS); //用来计算字符串 str 中连续有几个字符都属于字符串 accept，其原型为：
                                                           //size_t strspn(const char *str, const char * accept);
    int i;
    //如果传入一个本地的视频文件，协议就是file文件协议
    ...
    protocols = ffurl_get_protocols(NULL, NULL);  //找到黑白名单中的协议，这里都是null，因此需要遍历所有的协议
    if (!protocols)
        return NULL;
                ...
                const AVIOInterruptCB *int_cb)
{
    ...
    p = url_find_protocol(filename);  //查找协议
    if (p)   //如果找到协议
       return url_alloc_for_protocol(puc, p, filename, flags, int_cb); //为协议分配内存，并进行初始化赋值
                         ...
                         URLContext *parent)
{
    ...
    int ret = ffurl_alloc(puc, filename, flags, int_cb);  //filename表示输入的链接或路径
    if (ret < 0)
        return ret;
    if (parent)
    ...
    ret = ffurl_connect(*puc, options);   //建立协议链接，在里面打开协议uc->protocl->url_open
