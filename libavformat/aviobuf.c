...
int ffio_fdopen(AVIOContext **s, URLContext *h)
{
    ...
    buffer = av_malloc(buffer_size); //申请了一个读写缓冲buffer(结合文件协议，buffer大小为IO_BUFFER_SIZE)
    if (!buffer)
        return AVERROR(ENOMEM);
    ...
    //经过此函数之后，s->read_packet会调用ffurl_read，s->write_packet能调用ffurl_write
    *s = avio_alloc_context(buffer, buffer_size, h->flags & AVIO_FLAG_WRITE,
                             internal, io_read_packet, io_write_packet, io_seek); //分别对应ffurl_read,ffurl_write,ffurl_seek
    if (!*s)
        goto fail;
                        ...
                        )
{
    ...
    if (err < 0)
        return err;
	//申请一个AVIOContext对象并赋初值(部分初值来自ffurl_open_whitelist中创建的URLContext)
    err = ffio_fdopen(s, h);
    if (err < 0) {
        ffurl_close(h);
        return err;
