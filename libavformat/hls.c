                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    if (av_opt_get(in, "location", AV_OPT_SEARCH_CHILDREN, &new_url) >= 0)  //HTTP URL重定向
        url = new_url;
...
static int hls_read_header(AVFormatContext *s)  //playlist的解析，hls相关的初始化，提取节目信息
{
    HLSContext *c = s->priv_data;
    int ret = 0, i;
    ...
    if ((ret = parse_playlist(c, s->url, NULL, s->pb)) < 0)   //解析m3u8文件
        goto fail;
    ...
    /* 对于master playlist，逐个解析其中的playlist */          //需要在此进行不同分辨率的切换
    ...
    for (i = 0; i < c->n_variants; i++) {
		...
		(&program->metadata, "variant_bitrate", v->bandwidth, 0);
    }
