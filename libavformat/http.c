...
static int http_open_cnx_internal(URLContext *h, AVDictionary **options)
{
    ...
    ff_url_join(hoststr, sizeof(hoststr), NULL, NULL, hostname, port, NULL);  //解析出host地址

    proxy_path = s->http_proxy ? s->http_proxy : getenv("http_proxy");  //由于我们的http链接，基本都没用到proxy，所以use_proxy用不上
   ...
   //但在这里，由于传入了lower_proto（它表示http 协议的下一层协议，一般都是tcp，所以该值初始化的时候就是tcp）
   //所以buf的值是有lower_proto拼凑起来的tcp链接tcp:hostname:80，就是tcp+域名。
    ff_url_join(buf, sizeof(buf), lower_proto, NULL, hostname, port, NULL);

    if (!s->hd) {   //由于还没对hd进行赋值，所以一般为NULL,一般都会进入其中
        err = ffurl_open_whitelist(&s->hd, buf, AVIO_FLAG_READ_WRITE,
                                   &h->interrupt_callback, options,
                                   h->protocol_whitelist, h->protocol_blacklist, h);//这时通过url_find_protocol找到的就是tcp类型的URLProtocol,
                                                                                    //那么在ffurl_connect调用时调用的就是tcp.c里面的tcp_open函数，
                                                                                    //那么tcp的握手连接就在这个函数里面解析了
        if (err < 0)
            return err;
    }  //经过上述操作后，s->hd的值就是URLContext TCP
    //该函数的功能就是发送http request并等待http response
...
static int http_getc(HTTPContext *s)
{
    ...
    if (s->buf_ptr >= s->buf_end) {
        len = ffurl_read(s->hd, s->buffer, BUFFER_SIZE);   //只运行了三次  
        if (len < 0) {
            return len;
        } else if (len == 0) {
            return AVERROR_EOF;
                        ...
                        const char *proxyauth, int *new_location)
{
    ...
    //在调用ffurl_write以前的代码都是在填充http request的头部。

    //最终调用就是ret = send(s->fd, buf, size, MSG_NOSIGNAL);系统的send方法。所以最终都是
    //调用系统实现的Socket接口。至此，http_connect方法的发送request的请求就完毕了。剩下就是等待响应了。
    if ((err = ffurl_write(s->hd, s->buffer, strlen(s->buffer))) < 0) //调用的是tcp_write
        goto done;
	...
	/*
       ...
       line = Server: nginx/1.14.0
       line = Date: Wed, 05 Jun 2019 10:12:52 GMT
       line = Content-Type: video/mp2t
       line = Content-Length: 3644004
       line = Last-Modified: Wed, 05 Jun 2019 10:12:41 GMT
       line = Connection: keep-alive
       line = ETag: "5cf79599-379a64"
       line = Expires: Wed, 05 Jun 2019 10:12:51 GMT
       line = Cache-Control: no-cache
       line = Access-Control-Allow-Credentials: true
       line = Access-Control-Allow-Methods: GET, POST, DELETE, PUT, OPTIONS
       line = Access-Control-Allow-Headers: Accept, Authorization,DNT,X-CustomHeader,Keep-Alive,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type
       line = Accept-Ranges: bytes
	*/
    err = http_read_header(h, new_location);  //不断的读取网络返回的数据，并解析出来
    if (err < 0)
        goto done;
