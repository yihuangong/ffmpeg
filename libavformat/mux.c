...
static int interleave_packet(AVFormatContext *s, AVPacket *out, AVPacket *in, int flush)
{
    if (s->oformat->interleave_packet) {
        ...
        return ff_interleave_packet_per_dts(s, out, in, flush);   //将pkt加入到缓存队列中，然后依照dts的大小顺序取出
}
...
{
    int ret, flush = 0;
    //对包进行预处理，     检查流通道和包的pts，dts是否符合
    ret = prepare_input_packet(s, pkt);
    if (ret < 0)
        goto fail;
    //过滤器，也是区别于av_write_frame的地方
    if (pkt) {
        AVStream *st = s->streams[pkt->stream_index];
...
#if FF_API_COMPUTE_PKT_FIELDS2 && FF_API_LAVF_AVCTX
        if ((ret = compute_muxer_pkt_fields(s, st, pkt)) < 0 && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))  //计算pts,dts与duration
            goto fail;
#endif
    ...
    for (;; ) {
        AVPacket opkt;
        int ret = interleave_packet(s, &opkt, pkt, flush);  //与av_write_frame也不同
        if (pkt) {
