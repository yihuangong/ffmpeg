...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    /*遍历一个编解码器的链表，其首个结构是first_avcodec。编解码器链表是
      我们在av_register_all()函数中注册的。这个逐个匹配每一个编解码器的id，找到后返回对应的编解码器。*/
...
/* Open input file and probe the format if necessary. *///打开输入媒体（如果需要的话），初始化所有与媒体读写有关的结构体，比如  
    //AVIOContext，AVInputFormat
                      ...
                      AVDictionary **options)
{
    ...
    if (s->pb) {
        ...
        if (!s->iformat)
            return av_probe_input_buffer2(s->pb, &s->iformat, filename,  //如果已指定了pb但没指定iformat，以pb读取媒体数据进行探测取得iformat. 
    ...
    //如果没有设置AVInputFormat，那么使用av_probe_input_format2来判断文件格式。如果找到了大于预设值score的分数，那么直接返回分数
    if ((s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format2(&pd, 0, &score))))
        return score;
     //如果从文件名中也猜不出媒体格式，则只能打开这个文件进行探测了，先打开文件  ,io_open函数指向了options.c中的io_open_default函数
                        ...
                        AVInputFormat *fmt, AVDictionary **options)
{
    ...
    if (!s && !(s = avformat_alloc_context()))  //分配内存,在其中指定了s->io_open,指向的是io_open_default
        return AVERROR(ENOMEM);
    if (!s->av_class) {
    ...
    if (fmt)
        s->iformat = fmt;   //如果指定了输入文件的格式，就直接赋值
...
FF_ENABLE_DEPRECATION_WARNINGS
#endif
    if ((ret = init_input(s, filename, &tmp)) < 0)      //执行完此函数后，s->pb和s->iformat都已经指向了有效实例．pb是用于读写数据的，它  
    ...
    //黑名单，处理逻辑和白名单相同
    if (!s->protocol_blacklist && s->pb && s->pb->protocol_blacklist) {   //协议黑名单
    ...
    /* Allocate private data. *///为当前格式分配私有数据，主要用于某格式的读写操作时所用的私有结构．  
    ...
    //私有结构，根据流的数量分配流结构并初始化，把文件指针指向数据区开始处
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!st->need_parsing || !st->parser) {
            ...
            *pkt = cur_pkt;
            compute_pkt_fields(s, st, NULL, pkt, AV_NOPTS_VALUE, AV_NOPTS_VALUE);  //计算包的其他信息
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
	...
	/*probesize就是探测的大小，为了获得流的信息，这个函数会尝试的读一些包出来，然后分析这些数据，probesize限制了最大允许读出的数据的大小。*/
    ...
    /*
       这里定义了最大分析时长的限制。max_stream_analyze_duration 等于max_analyze_duration 等于30倍的timebase。
       此外，如果文件格式为flv或者Mpeg，那么他们会有各自的最大分析时长的限制。 
	*/
   ...
   /*第一次遍历流
     ...
     3.把解析器中的参数对应的拷贝到编解码器上下文环境中。调用的是avcodec_parameters_to_context函数
   */
    for (i = 0; i < ic->nb_streams; i++) {
        const AVCodec *codec;
        AVDictionary *thread_opt = NULL;
...
/*            if (!st->time_base.num)
        ...
        //st中有些参数确定，有些参数不确定  ，st->codec->codec->name没有，st->codec->codec_type有值
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE) && st->request_probe <= 0) {
            st->parser = av_parser_init(st->codecpar->codec_id);  //解析器为空，初始化解析器
            if (st->parser) {
        ...
        ret = avcodec_parameters_to_context(avctx, st->codecpar);  //将解析器中的参数拷贝到avctx中
        if (ret < 0)
            goto find_stream_info_err;
        if (st->request_probe <= 0)
            st->internal->avctx_inited = 1;

        codec = find_probe_decoder(ic, st, st->codecpar->codec_id);  //根据编码器的ID号查找编码器
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE              //一般直播没有字幕
            && codec && !avctx->codec) {
